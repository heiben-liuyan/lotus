...
RUST_LOG=info RUST_BACKTRACE=1 NETIP=$netip ./lotus-worker --repo=$repo --miner-repo=$miner_repo --storage-repo=$storage_repo --listen-addr="$netip:1283" --parallel-addpiece=0 --parallel-precommit1=0 --parallel-precommit2=0 --parallel-commit1=0 --parallel-commit2=0 --commit2-srv=true run &
pid=$!
