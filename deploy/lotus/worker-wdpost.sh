...
RUST_LOG=info RUST_BACKTRACE=1 NETIP=$netip ./lotus-worker --repo=$repo --miner-repo=$miner_repo --storage-repo=$storage_repo --listen-addr="$netip:1278" --id-file="~/.lotusworker/worker-wdpost.id" --parallel-addpiece=0 --parallel-precommit1=0 --parallel-precommit2=0 --parallel-commit1=0 --parallel-commit2=0 --wdpost-srv=true run &
pid=$!
