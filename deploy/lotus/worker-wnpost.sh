...
RUST_LOG=info RUST_BACKTRACE=1 NETIP=$netip ./lotus-worker --repo=$repo --miner-repo=$miner_repo --storage-repo=$storage_repo --listen-addr="$netip:1279" --id-file="~/.lotusworker/worker-wnpost.id" --parallel-addpiece=0 --parallel-precommit1=0 --parallel-precommit2=0 --parallel-commit1=0 --parallel-commit2=0 --wnpost-srv=true run &
pid=$!
