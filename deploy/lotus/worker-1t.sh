...
RUST_LOG=info RUST_BACKTRACE=1 NETIP=$netip ./lotus-worker --repo=$repo --miner-repo=$miner_repo --storage-repo=$storage_repo --listen-addr="$netip:1283" --id-file="~/.lotusworker/worker.1t" --max-tasks=14 --parallel-addpiece=14 --parallel-precommit1=14 --parallel-commit1=14 --parallel-commit2=0 run &
pid=$!
